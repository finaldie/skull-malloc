...
typedef struct {
    ...
    byte*      pmap;    // split index map to avoid cpu cache false sharing
...
static inline
void    f_cut_list(free_list* fl, fcut_list* fc, size_t count)
{
...
{
    fcut_list fc;
    size_t move_count = fl->gc_measure;
...
void*   _f_alloc(fl_mgr* pmgr, size_t size)
{
    ...
    if ( block ) {
        block->b_head.alloc_len = (unsigned int)size;
...
{
    if ( !ptr ) return skull_malloc(size);        // same as ANSI C
...
void*   skull_calloc(size_t num, size_t size)
{
    ...
    void* data = skull_malloc(size * num);
    if ( data )
